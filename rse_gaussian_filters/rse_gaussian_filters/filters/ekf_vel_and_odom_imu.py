...
from rse_observation_models.odometry_imu_observation_models import odometry_imu_observation_model_linearized
...
class KalmanFilter:
	...
	def __init__(self, initial_state, initial_covariance, proc_noise_std = [0.02, 0.02, 0.01], obs_noise_std = [0.02, 0.02, 0.01, 0.01, 0.01]):
		...
		# Observation model (C)
		self.h, self.H = odometry_imu_observation_model_linearized() # The observation model to use
	...
	def update(self, z, dt):
		...
		K = self.Sigma @ self.H(dt).T @ np.linalg.inv(self.H(dt) @ self.Sigma @ self.H(dt).T + self.Q)
		
		# Update state estimate (mu) 
		innovation = z - self.h(self.mu, self.prev_theta, dt)
		self.mu = self.mu + (K @ innovation).reshape((3,))
		# Update covariance (Sigma)
		I = np.eye(len(K)) # I = np.identity(len(self.mu_t))
		self.Sigma = (I - K @ self.H(dt)) @ self.Sigma
